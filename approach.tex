...
We set {\sc Wombat} to enumerate all solutions to the small instance, which are precisely the 
...
Given a seed (i.e., a $13 \times 13$ grid with some letters and $n$ black cells) we need to complete with an additional $26-n$ black cells\footnote{While having $26$ black cells is not a requirement for a legal crossword puzzle, having all allowed black cells ($26$ for a $13 \times 13$ grid) tends to maximize the score once the words are placed onto the grid.} 

...
m_lastProcessedNode(root), m_nodeThresholdForRestart(threshold) {
    ...
    this->m_discrepBudget = 100000;
    this->m_sterileBudget = 105;
}
...
DepthFirstSearch::~DepthFirstSearch() {
    ...
    cerr << "Search summary: " << endl << m_summaryInfo << endl;
    if (false) {
...
void DepthFirstSearch::runSearch() {
    ...
    m_root.setSkipDeadlockExtraction(false);
...
void DepthFirstSearch::processNode(const Node & node) {
    ...
    if (nr_them_points >= this->m_bestSolScore) {
        if (!node.getData().isLegal()) {
            cerr << "ILLEGAL STATE" << endl;
        cerr << "\n\033[1;31mNew partial solution with a high score found in DFS after ";
        cerr << getElapsedTime() << " seconds" << reset << endl;
        cerr << "Nr solutions: " << m_nrSolutionsFound;

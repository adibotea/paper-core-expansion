...
m_pathInfo(pathInfo), m_hitRemoteDP(false), m_allSuccessors(0)
{
}

Node::Node(const RandomWriteState & state, 
           unsigned int depth, unsigned int whichChild)
:m_data(state), m_depth(depth), m_whichChild(whichChild), m_pathInfo(""),
        m_allSuccessors(0)
{}
...
vector<Node> Node::generateSuccessors(int bestSolScore, 
    ...
    if (!this->getData().isLegal()) {
        cerr << "State to expand is deadlocked. Returning zero successors." << endl;
        return result;
    }

...
m_solFileName(""), m_maxTime(MAX_CPU_TIME), m_maxNodes(MAX_DFS_NODES), m_maxSolutions(
...
void ParamManager::displayUsage() const {
    cerr << "Usage: ./combus [-option value]+" << endl << "Options are:" << endl
            ...
            << "-plp 0/1 --> prune with payload ratio."
            ...
            << "-heur int_value --> heuristic to order successors."
            << endl << "  default: " << ParamManager::HEUR_ORDER << endl
            ...
            << "-search int_value --> type of processing."
            << endl << "  5 = DFS ; 15 = static analysis"
            << endl << "  default: " << ParamManager::SEARCH << endl
            << endl;
}
...
bool ParamManager::readParams(int argc, char * const argv[]) {
    ...
    while (it < argc) {
        ...
        if (option == "-i") {
            ...
            this->m_payloadPruning = atoi(argv[it + 1]);
            it += 2;
        } else if (option == "-dsa") {

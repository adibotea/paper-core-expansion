...
public:
    ...
    void addBlackPoint(int row, int col) {
        ...
        if (!hasCornerBP(row, col) && !adjacentBlackPoints(row, col)) {
            m_nrBPBlocks++;
        }
    }
    bool isGoal() const {
    ...
    string getTopSlots4NrpaMoves(int nr_slots) const {
        stringstream ss;
        int count = 0;
        for (int it = 0; it < this->m_themSlotOrder.size(); it++) {
            ...
            if (count > nr_slots)
                break;
            WordSlot slot = this->m_slots[this->m_themSlotOrder[it]];
            ss << "[ row: " << slot.getRow();
            ss << " ; col: " << slot.getCol();
            ss << " ; dir: " << (slot.getDirection() == HORIZONTAL?"H":"V");
            ss << " ] ";
        }
        return ss.str();
    }
    ...
    static void setNrpaScoreKind(unsigned int kind) {
        m_nrpaScoreKind = kind;
    };
    
    static string getNrpaScoreKind();
    ...
    const DeadlockRecord & getDeadlockCells() const {
        return m_deadlockCells;
    };

    DeadlockRecord getPartialDeadlock() const {
        return m_deadlockCells.difference(m_recentCellInstantiations);
    }
    ...
    bool matchesDeadlock(const vector<CellRecord> & deadlock) const;
...
protected:
    ...
    static unsigned int m_nrpaScoreKind;
    ...
    DeadlockRecord m_deadlockCells;
    ...
    bool adjacentBlackPoints(int row, int col) const {
        bool common_edge = (
                (row > 0 && m_grid[row - 1][col] == BLACKPOINT) ||
                (col > 0 && m_grid[row][col - 1] == BLACKPOINT) ||
                (row < m_nrRows - 1 && m_grid[row + 1][col] == BLACKPOINT) ||
                (col < m_nrCols - 1 && m_grid[row][col + 1] == BLACKPOINT));
        return common_edge;
    };
    bool hasCornerBP(int row, int col) const {
        bool common_corner = (
                (row > 0 && col > 0 && m_grid[row - 1][col - 1] == BLACKPOINT) ||
                (row < m_nrRows - 1 && col > 0 && m_grid[row + 1][col - 1] == BLACKPOINT) ||
                (row < m_nrRows - 1 && col < m_nrCols - 1 && m_grid[row + 1][col + 1] == BLACKPOINT) ||
                (row > 0 && col < m_nrCols - 1 && m_grid[row - 1][col + 1] == BLACKPOINT));

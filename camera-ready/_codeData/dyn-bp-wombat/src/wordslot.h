...
public:
     ...
     * 
     */
    void computeDomainDBP(const vector<Dictionary> &dics,
            bool non_them_ok,
    ...
    void computeImpactAreaSize(const vector<WordSlot> & slots);
    ...
    /*
     * Compute the numbers of matching words from dictionaries,
     * including words of a shorter length.
     * Argument non_them_ok tells whether it's ok to allow non-thematic
     * dictionaries for slots of length at least MIN_LENGTH_NTW
     */
    void setNrMatchingWordsFromDics(const vector<Dictionary> & dics,
            bool non_them_ok,
            vector<int> & allowed_bps,
            bool bp_allowed);
    ...
    bool isPrefixDeadlocked(vector<int> & legal_bps) const {
        if (this->getLength() <= 2)
            return false;
        if (legal_bps.size() > 0 && legal_bps[0] <= 2)
            return false;
        return (this->getNrMatchingWords() == 0);
    }
